...
const iniDates = ({ step, type, value }: Pick<Props, 'step' | 'type' | 'value'>) =>
	Array(...Array(DATE_LENGTH)).map((date, index) =>
		nextMap[type](value, (index - MIDDLE_INDEX) * step),
	);

const DatePickerItem: FC<Props> = ({ type, value, min, max, format, step, onSelect }) => {
	...
	const updateDates = (direction: Direction) => {
		if (direction === Direction.UP) {
			...
			setDates([...dates.slice(1), nextMap[type](dates[dates.length - 1], step)]);
		} else {
			currentIndex.current--;
			setDates([nextMap[type](dates[0], -step), ...dates.slice(0, dates.length - 1)]);
		}
	...
	const handleStart = (
		...
		touchY.current = isTouchEvent(event) ? event.targetTouches[0].pageY : event.pageY;
	...
	const handleMove = (
		...
		const nextTouchY = isTouchEvent(event) ? event.targetTouches[0].pageY : event.pageY;
	...
	const handleContentTouch: React.TouchEventHandler<HTMLDivElement> = (event) => {
		event.type === 'touchend' && event?.cancelable && event?.preventDefault();
	...
	useEffect(() => {
		if (mouseDown) {
			document.addEventListener('mousemove', handleContentMouseMove, { passive: false });
			document.addEventListener('mouseup', handleContentMouseUp, { passive: false });
			return () => {
	...
	const renderDatepickerItem = useCallback(
	...
	return (
		<div className='vuivui-datepicker-col-1'>
			{/* eslint-disable-next-line jsx-a11y/no-static-element-interactions */}
			<div
				className='vuivui-datepicker-viewport'
				onTouchStart={handleContentTouch}
				onTouchMove={handleContentTouch}
				onTouchEnd={handleContentTouch}
				onMouseDown={handleContentMouseDown}
			>
				<div className='vuivui-datepicker-wheel'>
					<div
						className={`vuivui-datepicker-scroll ${isAnimating ? 'active' : ''}`}
						style={scrollStyle}
					>
...
export default React.memo(
	DatePickerItem,
	(prevProps, nextProps) => prevProps.value.getTime() === nextProps.value.getTime(),
);
