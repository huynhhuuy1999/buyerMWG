...
import { numberWithCommas, roundNumber } from 'utils';
...
const FilterPrice: React.FC<IFilterPrice> = ({
	...
	}, [refSlider1, refSlider2, refTrack, defaultSelected, maxPrice]);
		...
		if (refSlider2.current && refSlider1.current) {
			...
			if (refPrice2.current) {
				if (Number(refSlider2.current.value) >= 1000000) {
					const numberRounded = roundNumber(Number(refSlider2.current.value) / 1000000, 2);
					refPrice2.current.innerHTML = `${numberRounded} triệu`;
				} else {
