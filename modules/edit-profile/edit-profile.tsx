...
const ModalEditProfile = dynamic(() => import('./components/ModalEditProfile'), {
	loading: () => <>Loading...</>,
	ssr: false,
});

const ModalPreviewAvatar = dynamic(() => import('./components/ModalPreviewAvatar'), {
	loading: () => <>Loading...</>,
	ssr: false,
});
...
const EditPage = () => {
	...
	const handleClosePopup = () => {
		setShowModal(false);
		setVerifyOTP({ ...verifyOTP, phoneNumber: '', type: '', verifyId: 0 });
	};
	...
	return (
		<Fragment>
			...
			>
				<div className={`flex h-full flex-col bg-[#F6F6F6] pt-[100px] pb-[62px] rounded-[8px]`}>
					<div className='py-auto relative mx-6 mb-0 rounded-[8px] bg-white px-20'>
						<Controller
							render={({ field }) => (
								<Fragment>
									<div
										...
										<>
											...
											<div
												className={classNames([
													'absolute top-[42px] left-[calc(50%-64px)] z-[7] mt-2 w-[200px] origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none',
													isDropdownAvatar
						...
						{/* eslint-disable-next-line tailwindcss/enforces-negative-arbitrary-values */}
						<div className='absolute -top-[95px] right-[0] flex justify-end py-2'>
							{isSaving !== undefined ? (
								isSaving ? (
						...
						<div className='z-0 flex h-[128px] max-w-4/10 flex-4/10 justify-between pb-12 pt-20'>
							<Controller
								render={({ field }) => (
									<>
										<RadioField
								...
								<div className='relative pb-6'>
									<button
										...
										onClick={() => {
											if (field.value) {
												setShowModal(true);
												setVerifyOTP({ ...verifyOTP, type: 'CHANGE' });
											} else {
												setShowModal(true);
												setVerifyOTP({ ...verifyOTP, type: 'NEW' });
											}
										}}
									>
						...
						{/* <div className='mt-auto ml-auto w-[90px] bg-white py-3'>
							<button
