...
const HeaderChat: React.FC<IHeaderChatProps> = ({
	...
	return (
		<div className='relative flex items-center rounded border border-gray-200 bg-[#F05A94] p-2'>
			<div
				className='justify-left flex w-4/5 cursor-pointer items-center'
				role='button'
				tabIndex={0}
			...
			>
				...
				/>
				<div className='flex items-center px-2 text-sm font-semibold text-white hover:bg-white hover:text-current'>
					<div className='mr-2 block'>
						<div>
							<span className='mx-2 block'>{conversation?.name && conversation.name}</span>
						</div>
						<div>
							<span className='ml-2 block text-xs font-thin'>Đang hoạt động</span>
						</div>
					</div>
					<ImageCustom
