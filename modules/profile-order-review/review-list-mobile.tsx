...
const DynamicModalOrderMobile = dynamic(
	() => import('../profile-modal-order-review/modal-order-evalution-mobile'),
	{ ssr: false, loading: () => Skeletons },
);

const ReviewListMobile: React.FC = () => {
	...
	return (
		<>
			<div className='shadow-[rgba(14, 14, 16, 0.06)] rounded-md bg-white p-[20px]'>
				{listOrderEvalution?.data?.length ? (
							...
							? listOrderEvalution.data.map((itemOrder: any, index: number) => (
									<div key={index} className='flex items-center justify-center'>
										<div className='relative flex w-full flex-col rounded-md border border-[#cdcdcd]'>
											<div
												onClick={() => handleCancelOrder?.(typeOrder.MERCHANT, itemOrder, 0)}
												className='absolute right-[-17px] top-[-17px] flex h-8 w-8 cursor-pointer items-center justify-center rounded-[50%] bg-[rgba(143,155,179,0.7)]'
												aria-hidden={true}
											>
												<ImageCustom
					...
					<DynamicEmpty content='Danh sách chờ đánh giá trống' />
				)}
				{isModal && dataOrder && (
					<DynamicModalOrderMobile
						isModal={isModal}
